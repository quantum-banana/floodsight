...
export function CommandCenter() {
  ...
  return (
    <div className="arctic-shell min-h-screen text-slate-100">
      ...
      />
      <main className="relative z-10 mx-auto max-w-[1600px] px-3 py-3 sm:px-4 lg:px-5">
        ...
        {media.mode === "SIMULATION" && <section className="glass-surface relative mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl px-3 py-2.5 sm:px-4" aria-label="Simulation status and controls">
        ...
        <div className="relative grid min-w-0 items-start gap-3 xl:grid-cols-[minmax(0,3fr)_minmax(20rem,1fr)]">
          ...
          {snapshot
            ? <div className="grid min-w-0 content-start gap-3"><PriorityList zones={snapshot.zones} route={snapshot.route} selectedZoneId={selectedZoneId} onSelectZone={setSelectedZoneId} /><IncidentOverview snapshot={snapshot} connectionState={connectionState} /></div>
            : <PendingIntelligence label="Incident analytics will appear after backend inference completes." />}
        </div>

        {snapshot ? (
          <>
            <div className="relative mt-3 grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1.75fr)_minmax(19rem,0.75fr)]">
