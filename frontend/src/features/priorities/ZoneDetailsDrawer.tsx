...
import type { AggregateMetricAvailability, VideoPriorityObservation } from "../../types/ingestion";
...
interface ZoneDetailsDrawerProps {
  ...
  buildingDamageCountAvailability?: AggregateMetricAvailability;
}
...
export function ZoneDetailsDrawer({ zone, onClose, onFocusMap, observation = null, segmentationEvidenceAvailable = true, detectionEvidenceAvailable = true, buildingDamageCountAvailability = "AVAILABLE" }: ZoneDetailsDrawerProps) {
  ...
  const damageCountAvailability = observation?.building_damage_count_availability ?? buildingDamageCountAvailability;
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/55 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <aside role="dialog" aria-modal="true" aria-labelledby="zone-drawer-title" className="drawer-enter flex h-full w-full max-w-md flex-col border-l border-white/[0.1] bg-[#09151c] shadow-[-30px_0_80px_rgba(0,0,0,.45)]">
        <div className="flex items-start justify-between gap-4 border-b border-white/[0.07] p-5">
          <div><p className="eyebrow">Rescue zone detail</p><h2 id="zone-drawer-title" className="mt-2 text-xl font-semibold text-white">{zone.display_name}</h2><p className="mt-1 font-mono text-[0.66rem] text-cyan-300">{zone.zone_id}</p><p className="mt-1 text-xs text-slate-500">Rank #{zone.rank} · Updated {formatTimestamp(zone.updated_at_ms)} UTC</p>{observation && <p className="mt-1 text-xs font-semibold text-amber-200">Observed at {formatMediaTime(observation.media_time_ms)} · source frame {observation.source_frame_id}</p>}</div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {strandedAlert && hasDetectionEvidence && (
            ...
            >
              ...
              <dl className="mt-3 grid grid-cols-3 gap-2">
                ...
                <AlertFact label="Flood exposure" value={hasSegmentationEvidence ? strandedAlert.flood_exposure : "UNAVAILABLE"} />
                <AlertFact label="Primary access" value={hasSegmentationEvidence ? strandedAlert.primary_access : "UNAVAILABLE"} />
              </dl>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.06]">
            <Fact label="People" value={hasDetectionEvidence ? zone.people_count : "Unavailable"} />
            <Fact label="Vehicles" value={hasDetectionEvidence ? zone.vehicle_count : "Unavailable"} />
            <Fact label="Flood coverage" value={hasSegmentationEvidence ? formatPercent(zone.flood_coverage_percent) : "Unavailable"} />
            <Fact label="Building instances" value={damageCountAvailability === "AVAILABLE" ? zone.building_damage_count : damageCountAvailability === "NOT_SUPPORTED" ? "Not supported" : "Unavailable"} />
            <Fact label="Damage coverage" value={hasSegmentationEvidence ? formatPercent(zone.building_damage_coverage_percent ?? 0) : "Unavailable"} />
            <Fact label="Pool coverage" value={hasSegmentationEvidence ? formatPercent(zone.pool_coverage_percent ?? 0) : "Unavailable"} />
            <Fact label="Road condition" value={hasSegmentationEvidence ? (zone.road_condition === "UNKNOWN" ? "UNCERTAIN" : zone.road_condition) : "Unavailable"} />
            <Fact label="Access status" value={hasSegmentationEvidence ? zone.access_status : "Unavailable"} />
            <Fact label="Grid cells" value={zone.grid_cells?.join(", ") || "Not supplied"} />
            <Fact label="Temporal samples" value={zone.temporal_samples ?? 1} />
            <Fact label="Temporal state" value={zone.stale ? "STALE" : "CURRENT"} />
          </dl>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-white/[0.07] p-4">{observation ? <p role="note" className="rounded-lg border border-amber-300/15 bg-amber-300/[0.05] px-3 py-2 text-[0.66rem] leading-5 text-amber-100">Historical video evidence. The map remains on the latest analyzed frame.</p> : <button type="button" onClick={onFocusMap} className="command-button command-button-primary"><Icon name="focus" />Focus on map</button>}<button type="button" onClick={onClose} className="command-button command-button-secondary">Close</button></div>
      </aside>
    </div>
  );
}
