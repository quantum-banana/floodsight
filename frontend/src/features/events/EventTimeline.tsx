  ...
  return (
    <section className="command-panel min-w-0" aria-labelledby="events-heading">
      ...
      <div className="max-h-64 overflow-y-auto p-2 sm:p-3">
        {uniqueEvents.length ? <ol className="divide-y divide-white/[0.05]">{uniqueEvents.map((event) => <li key={event.event_id} className="grid grid-cols-[4.5rem_1fr] gap-3 px-2 py-3 sm:grid-cols-[5.5rem_6.5rem_1fr]"><time dateTime={new Date(event.timestamp_ms).toISOString()} className="font-mono text-[0.68rem] text-slate-500">{formatTimestamp(event.timestamp_ms)}</time><span className={`hidden w-fit rounded-md border px-2 py-0.5 text-[0.58rem] font-bold tracking-wider uppercase sm:inline ${eventTone[event.severity]}`}>{event.category}</span><p className="text-xs leading-5 text-slate-300">{event.message}</p></li>)}</ol> : <EmptyState label="No incident events in this snapshot." />}
      </div>
    </section>
  );
}
